# unified_astro_logger.py (v13 - Final)
#
# Changelog:
...
# - Supports clean shutdown via 'q' + Enter or Ctrl+C.
...
class UnifiedAstroLogger:
    ...
    def _resolve_dynamic_paths(self):
        ...
        for key in ["FOCUS_LOG_FILE", "IMAGE_BASE_DIR", "NINA_LOG_DIR", "BOLTWOOD_FILE_PATH", "MASTER_DARK_DIR", "MASTER_FLAT_DIR"]:
            if self.config[key]:
    ...
    def start(self):
        ...
        logging.info("Starting all threads. Press 'q' then Enter, or Ctrl+C to stop.")
        for t in threads:
            t.start()
        try:
            for t in threads:
